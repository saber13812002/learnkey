...
if ($attachment_ids) {
		...
		foreach ( $attachment_ids as $id ) {
			...
			if( get_option( 'woocommerce_enable_lightbox' ) == 'yes' ) {
                printf( '<a href="%s" title="%s" rel="prettyPhoto[product-gallery]" class="%s">%s</a>', wp_get_attachment_url( $id ), esc_attr( get_the_title( $id ) ), implode(' ', $classes), wp_get_attachment_image( $id, apply_filters( 'single_product_small_thumbnail_size', 'shop_thumbnail' ), 0, $style ) );
            } else {
