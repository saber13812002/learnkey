...
class YIT_Metabox {
    ...
    public function register_metaboxes() {
        foreach( $this->_metaboxes as $id => $metabox ) 
            { add_meta_box( $id, $metabox['title'], array( &$this, 'metaboxes_html' ), $metabox['post_type'], 'normal', 'high', array( 'metabox_id' => $id ) ); }
    }
     ...
     * @since 1.0.0
     */
    public function register_metabox( $id, $title, $post_type ) {
...
if( !function_exists( 'yit_register_metabox' ) ) {
     ...
     * @since 1.0.0
     */
    function yit_register_metabox( $id, $title, $post_type  ) {
        $metabox = yit_get_model( 'metabox' );
        $metabox->register_metabox( $id, $title, $post_type );
    }
}
