...
function superstore_custom_cart_button_text() {
    return __('افزودن به سبد خرید', 'woothemes');
}
		...
		<nav class="account-links">
			<ul>
				<?php if ( woocommerce_get_page_id( 'myaccount' ) !== -1 ) { ?>
					<li class="my-account"><a href="<?php echo $url_myaccount; ?>" class="tiptip" title="حساب کاربری"><span><?php if ( is_user_logged_in() ) { _e('My Account', 'woothemes' ); } else { _e( 'Log In', 'woothemes' ); } ?></span></a></li>
				<?php } ?>

				<?php if ( ! is_user_logged_in() && woocommerce_get_page_id( 'myaccount' ) !== -1 && get_option('woocommerce_enable_myaccount_registration')=='yes' ) { ?>
					<li class="register"><a href="<?php echo $url_myaccount; ?>" class="tiptip" title="حساب کاربری"><span><?php _e( 'Register', 'woothemes' ); ?></span></a></li>
				<?php } ?>

				<?php if ( is_user_logged_in() ) { ?>

					<?php if ( woocommerce_get_page_id( 'edit_address' ) !== -1 ) { ?>
						<li class="edit-address"><a href="<?php echo $url_editaddress; ?>" class="tiptip" title="تغییر آدرس"><span><?php _e( 'Edit Address', 'woothemes' ); ?></span></a></li>
					<?php } ?>

					<?php if ( woocommerce_get_page_id( 'change_password' ) !== -1 ) { ?>
						<li class="edit-password"><a href="<?php echo $url_changepass; ?>" class="tiptip" title="تغییر رمز عبور"><span><?php _e( 'Change Password', 'woothemes' ); ?></span></a></li>
					<?php } ?>

					<?php if ( woocommerce_get_page_id( 'view_order' ) !== -1 ) { ?>
						<li class="logout"><a href="<?php echo wp_logout_url( get_permalink() ); ?>" class="tiptip" title="خروج"><span><?php _e( 'Logout', 'woothemes' ); ?></span></a></li>
					<?php } ?>
